...
import { useDepositMoneyMutation } from "@/redux/features/walletUser/walletUser.api";
...
export const UserDeposit = () => {
  ...
  const onSubmit = async (data: z.infer<typeof depositMoneySchema>) => {
    const cashInData = {
      userNo: data.userNo,
      amount: data.amount,
    ...
    try {
      await depositMoney(finalData).unwrap();
